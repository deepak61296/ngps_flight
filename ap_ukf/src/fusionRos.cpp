...
using namespace std::chrono_literals;
  ...
  FusionRos::FusionRos(const rclcpp::NodeOptions & options)
    ...
    // No need to declare the parameters here if already decalred in param file.
//    declare_parameter("frequency", 0.0);
//    declare_parameter("process_noise_covariance", 0.0);
//    declare_parameter("initial_estimate_covariance", 0.0);
    ...
    try {
      ...
      dvlSub_ = this->create_subscription<geometry_msgs::msg::TwistWithCovarianceStamped>("/eca_a9/dvl_twist", 50,
                                      std::bind(&FusionRos::dvlCallback, this, _1));
      depthSub_ = this->create_subscription<sensor_msgs::msg::FluidPressure>(
          "/eca_a9/pressure", 50,
          std::bind(&FusionRos::depthCallback, this, _1));

      gpsSub_ = this->create_subscription<nav_msgs::msg::Odometry>(
          "/odometry/gps", 50,
          std::bind(&FusionRos::gpsCallback, this, _1));
...
void FusionRos::dvlCallback(const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr msg) {
  MeasurementPtr z = std::make_shared<Measurement>(VEL_SIZE);
  z->type = MeasurementTypeVel;
  auto& stamp = msg->header.stamp;
  rclcpp::Time time(stamp.sec, stamp.nanosec);
  z->time = time.seconds();
  tf2::Transform targetFrameTrans = getTransformFrame(msg->header);

  tf2::Vector3 linVel(msg->twist.twist.linear.x, msg->twist.twist.linear.y, msg->twist.twist.linear.z);
  ...
  // rotate covariance matrix to base_link
  Eigen::MatrixXd covarianceRotated(3, 3);
  rotateCovariance(&(msg->twist.covariance[0]),
                   targetFrameTrans.getRotation(), covarianceRotated);
...
void FusionRos::gpsCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
...
void FusionRos::depthCallback(const sensor_msgs::msg::FluidPressure::SharedPtr msg) {
...
tf2::Transform FusionRos::getTransformFrame(
  ...
  try {
    tf2::fromMsg(tfBuffer_->lookupTransform(baseLinkFrame_, header.frame_id,
                 ...
                 targetFrameTrans); // TODO : Check if duration is in seconds
