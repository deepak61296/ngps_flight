# Fusion
Robot fusion using unscented Kalman filter. This package does not stink :)

* Output state message param: `/topic/sensor/odom_state `
* Output frame: `temp_odom`

## Run
* Node: `roslaunch ap_ukf localizer.launch`
* Tests: `catkin_make run_tests_ap_ukf`
...
* `fusion(.h .cpp)` - implements a fusion filter using UKF; process and measurement models defined here
* `fusion_ros(.h .cpp)` - ROS wrapper for fusion; implements all ROS inputs and outputs for fusion
* `fusion_node.cpp` - runs FusionRos class as a ros node
